...
// 피드 생성
export async function createFeed(feedData: Omit<PhotoFeed, 'feed_id' | 'created_at'>) {
...
// 피드 업데이트
export async function updateFeed(feedId: string, updates: Partial<Omit<PhotoFeed, 'feed_id' | 'created_at'>>) {
  const { data, error } = await supabase
    .from('TB_PHOTO_FEED')
    .update(updates)
    .eq('feed_id', feedId)
